--can we trace dofile like this?
local _dofile = dofile
dofile = function(x)
    local dbginfo = debug.getinfo(1, 'S')
    local source_file = (dbginfo and dbginfo.source) or "(???)"
    print_error("" .. source_file .. ": dofile( \"" .. (x or "(nil)") .. "\")")
    _dofile(x)
end

local _dofile_once = dofile_once
dofile_once = function(x)
    local dbginfo = debug.getinfo(1, 'S')
    local source_file = (dbginfo and dbginfo.source) or "(???)"
    print_error("" .. source_file .. ": dofile_once( \"" .. (x or "(nil)") .. "\")")
    _dofile_once(x)
end
