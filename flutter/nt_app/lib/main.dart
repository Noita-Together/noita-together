...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<TwitchTokens?> initTokens() async {
    return _getPrefs().then((SharedPreferences value) async {
      if (!value.containsKey('auth')) return null;
  ...
  void initState() {
    super.initState();
    Future.wait([initTokens(), getGamePath()]).then((results){
      setState(() {
        fetchedLoginFromDisk = true;
        twitchTokens = results[0] as TwitchTokens?;
        fetchingGamePath = false;
        gamePath = results[1] as String?;
      });
    });
  }
  ...
  Widget renderGamepathInfo(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: const Column(
        ...
        children: <Widget>[Text("Please select an option below")],
      ),
      actions: [
        TextButton(
